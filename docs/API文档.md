...
| HTTP 状态码 | 响应 `code` | 说明 | 典型场景 |
| --- | ---: | --- | --- |
| 200 | 0 | 成功 | 查询成功、预约成功、取消成功 |
| 400 | 400 | 业务规则校验失败 | 重复预约、号源已满、取消超时 |
| 401 | 401 | 未登录或登录失效 | 未携带 Token、Token 过期或非法 |
| 403 | 403 | 无权限 | 患者账号访问管理员接口 |
| 400 | 422 | 请求参数校验失败 | 必填字段为空、手机号格式错误 |
| 500 | 500 | 系统异常 | 未预期服务端错误 |
...
| HTTP 状态码 | 响应 `code` | `message` | 场景 |
| --- | ---: | --- | --- |
| 400 | 400 | 当前号源不可预约 | 已约满、停诊或剩余号源为 0 |
| 400 | 400 | 同一就诊人同一科室同一天只能预约一次 | 重复预约 |
| 400 | 400 | 只能预约未来7天内号源 | 号源日期不符合规则 |
